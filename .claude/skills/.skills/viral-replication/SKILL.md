...
- Add scroll-stopping text overlay using `scripts/add-photo-overlay.py`
- The overlay adds: dark gradient, bold hook text, lime-green highlights, AI 5 PHÚT brand pill

```bash
python3 scripts/add-photo-overlay.py \
  --photo context/images/photo.jpg \
  --text "Hook text goes here — key phrase viewers must stop for" \
  --highlight "KEY PHRASE" "NUMBERS" \
...
- `--text`: use the post's hook (first 1-2 lines) — must be short enough to read at a glance
- `--highlight`: words to color lime green — numbers, verbs, superlatives (e.g. "3 BƯỚC", "X5", "90%")
- `--position bottom`: hook at bottom (most common), `top` for sky/landscape photos, `center` for dramatic single-subject shots
- Keep text under ~12 words for legibility at mobile scroll speed
- NEVER overlay text on a face — pick a position that avoids the subject's face
