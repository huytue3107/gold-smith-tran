#!/usr/bin/env python3
"""
Build an HTML dashboard showing all Gold Smith posts in the posts/ directory.
...
def render_card(slug, post_text, visual_html):
    post_text_html = post_text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\n", "<br>")
    post_num = slug.split("-")[0] if slug and slug[0].isdigit() else slug
...
def build_card(post_dir):
    ...
    return render_card(slug, post_text, visual_html)


def build_markdown_card(post_md):
    slug = os.path.splitext(os.path.basename(post_md))[0]
    _, post_text = parse_post_md(post_md)
    return render_card(slug, post_text, '<span class="no-visual">text</span>')
...
def build_html():
    ...
    for post_file in post_files:
        cards.append(build_markdown_card(post_file))
...
<head>
    ...
    <title>Gold Smith Content Dashboard</title>
    <style>
...
<body>
    <h1>Gold Smith Content Dashboard</h1>
    {rows_html}
